...
interface AssemblyData {
    assembly: Assembly;
    stats: SurnameSimilarStat[];
}

const SurnameSimilarSearch: React.FC = () => {
    ...
    const [paginationState, setPaginationState] = useState<Record<number, number>>({}); // Track page for each assembly
    ...
    const handleSearch = async () => {
        setLoading(true);
        setPaginationState({}); // Reset all pagination on new search
        try {
            let assemblyParam = '';
            if (selectAll) {
                assemblyParam = 'all';
            ...
            if (viewMode === 'combined') {
                toast.success('Combined surname data loaded successfully!');
            } else {
                toast.success('Surname  statistics loaded successfully!');
            }
        } catch (error: any) {
    ...
    // Pagination logic - each assembly has its own page
    const getPaginatedData = (stats: SurnameSimilarStat[], assemblyId: number) => {
    ...
    const getTotalPages = (stats: SurnameSimilarStat[]) => {
        return Math.ceil(stats.length / itemsPerPage);
    };

    const setAssemblyPage = (assemblyId: number, page: number) => {
        setPaginationState(prev => ({ ...prev, [assemblyId]: page }));
    };

    // Apply filters to stats
    const applyFilters = (stats: SurnameSimilarStat[]): SurnameSimilarStat[] => {
        if (filterType === 'none') {
            return stats;
        }
            ...
            <div className="max-w-7xl mx-auto bg-white shadow-xl rounded-lg overflow-hidden">
                <div className="bg-gradient-to-r from-green-600 to-teal-600 p-6">
                    <div className="flex items-center justify-between">
                        ...
                        <h1 className="text-3xl font-bold text-white flex-1 text-center">Surname Search</h1>
                ...
                <div className="p-8 space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
                        ...
                        <div className="bg-gradient-to-r from-green-600 to-teal-600 px-6 py-4">
                            ...
                            <p className="text-green-100 text-sm mt-1">Choose one or more assemblies to view statistics</p>
                        </div>
                        ...
                        <div className="p-6 space-y-5">
                                        ...
                                        <div className="border-2 border-gray-300 rounded-lg p-2 bg-white">
                                            ...
                                            <div className="relative mb-2">
                                                <input
                                                    ...
                                                    className="w-full px-3 py-1.5 pl-8 text-sm border border-gray-300 rounded focus:border-green-500 focus:ring-1 focus:ring-green-200"
                                                />
                            ...
                            {!selectAll && selectedAssemblies.length > 0 && (
                                <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                                    ...
                                    <span className="text-sm font-semibold text-green-900">
                                        <span className="text-green-600 text-lg">{selectedAssemblies.length}</span> {selectedAssemblies.length === 1 ? 'assembly' : 'assemblies'} selected
                                    </span>
                                </div>
                            )}
                                ...
                                {filterType === 'top' && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <input
                                            ...
                                            min="1"
                                            placeholder="10"
                                            className="w-24 px-3 py-1.5 text-sm border border-gray-300 rounded focus:border-green-500 focus:ring-1 focus:ring-green-200"
                                        />
                                ...
                                {filterType === 'range' && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <input
                                            ...
                                            className="w-32 px-3 py-1.5 text-sm border border-gray-300 rounded focus:border-green-500 focus:ring-1 focus:ring-green-200"
                                        />
                            ...
                            <div className="flex justify-end">
                                <button
                                    ...
                                    className="px-4 py-2 bg-green-600 text-white text-sm font-medium rounded hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                                >
                                    {loading ? (
                                        <>
                    ...
                    {results.length > 0 && (
                        <div className="mt-8 border-t pt-6">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-2xl font-bold text-gray-800">Surname  Statistics</h2>
                                <span className="bg-green-100 text-green-800 py-2 px-4 rounded-full font-semibold">
                                            ...
                                            <div className="bg-gradient-to-r from-green-500 to-teal-500 p-4">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <div className="bg-opacity-20 backdrop-blur-sm px-4 py-2 rounded-lg">
                                                        <p className="text-white text-sm font-semibold">Total Surnames</p>
                                                        <p className="text-white text-2xl font-bold">
                                                            {filteredStats.length.toLocaleString()}
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <table className="min-w-full bg-white">
                                                    ...
                                                    <tbody className="divide-y divide-gray-200">
                                                        {paginatedStats.map((stat, idx) => {
                                                            ...
                                                            return (
                                                                <tr key={idx} className="hover:bg-green-50 transition duration-150">
                                                                    <td className="py-4 px-6 text-sm text-gray-500 font-mono">
                                                                        {actualIndex}
                                                                    </td>
                                                                    <td className="py-4 px-6 text-sm font-semibold text-gray-900">
                                                                        {stat.surname_similar}
                                                                    </td>
                                                                    <td className="py-4 px-6 text-sm text-right font-bold text-green-700">
                                                ...
                                                {totalPages > 1 && (
                                                    <div className="mt-4 flex items-center justify-between border-t pt-4">
                                                        <div className="text-sm text-gray-600">
                                                            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredStats.length)} of {filteredStats.length} entries
                                                        </div>
                                                        <div className="flex gap-2">
                                                            <button
                                                                onClick={() => setAssemblyPage(assemblyId, Math.max(1, currentPage - 1))}
                                                            ...
                                                            <button
                                                                onClick={() => setAssemblyPage(assemblyId, Math.min(totalPages, currentPage + 1))}
