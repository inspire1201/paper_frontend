...
class SurnameService {
     ...
     */
    async getCategoryStats(assemblyId: string, viewMode: 'separate' | 'combined' = 'separate') {
     ...
     */
    async getCasteDetails(category: string, assemblyId?: number | string) {
     ...
     * @param viewMode - 'separate' or 'combined'
     */
    async getCasteStats(assemblyId: string, viewMode: 'separate' | 'combined' = 'separate') {
     ...
     * @param assemblyId - Assembly ID (can be number, string of IDs, or 'all')
     */
    async getSurnameDetails(caste: string, assemblyId?: number | string) {
        // If assemblyId is 0 or undefined, use 'all'
        const assembly_id = assemblyId === 0 || !assemblyId ? 'all' : assemblyId;
     ...
     * @param viewMode - 'separate' or 'combined'
     */
    async getSurnameSimilarStats(assemblyId: string, viewMode: 'separate' | 'combined' = 'separate') {
     ...
     */
    async getCasteAssemblyDistribution(casteName: string) {
